...
There is an extension in progress for OWT to store metadata at the feature level: https://github.com/CesiumGS/glTF/pull/1[link]
This extension currently supports storing per-vertex metadata, but it could possibly be expanded to support per-face metadata and used in CDB. 
...
image::images/TankModel_nodes_OpenFlight.png[width=300] image::images/TankModel_nodes_glTF.jpg[width=300]
