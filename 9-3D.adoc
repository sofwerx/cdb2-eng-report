...
. 1- The mesh solution merges many CDB datasets into one. The elevation and 3D features along with the imagery are now all merged into one dataset.
. 2- The attribution to objects on the terrain is currently done in the vector, with a reference to the geometry. I a mesh case, the linkage will likely need to be revered and have the geometry link to the attribution.
