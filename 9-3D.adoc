...
. Models be stored as files or in GeoPackage 
. Models preferred to be glTF (Later consider I3S, 3D Tiles, CityGML and other encodings)
...
. LOD Structure? (MS LOD extention for glTF) (Editors note: `Shouldn't the same LoD structure be used throughout CDB-X? Otherwise, things get confusing. - answer: Models LODs cout vertex, sig size etc... and have their own LOD table which is not the same as tiled LODs`)
...
..    Elevation
..    Imagery
..    Polygon features. (previously aerial)
..    linear features
...
=======
This is one of the larger challenges encountered during our investigations. In the past there was discussion about including an external reference extension in glTF but it was decided not to include it {*Who made this decision?*}.
In past cases, separate index files were used to reference multiple glTF files from a common source. This is sufficient for some cases but does not help for the use case of sharing geometry between multiple models. For example sharing common roof clutter objects between different buildings
...
=== Investigation 2 - Merging all geometry in a tile {*I am assuming this section will get more information - and an edit*}
This investigation was not done by the 3D group but the tiling group did experiement. A few different ways to encode are possible, each with their own implications.
